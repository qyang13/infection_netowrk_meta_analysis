...
lfc_mat=read.table("./deseq/res/meta_log2FC_mat_v2.txt",header = T)
padj_mat=read.table("./deseq/res/meta_padj_mat_v2.txt",header = T)
...
keep=which(rowMin(as.matrix(padj_mat))<0.001)
...
write.table(rnk_mat,"./deseq/res/rank_matrix_padj_cutoff_0001.txt",row.names = T, col.names = T, quote = F, sep = "\t")
